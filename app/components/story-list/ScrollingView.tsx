...
const storyViewData: ItemData[] = [
  {
    ...
    title:
      '청년 시절 가장 기억에 남는 순간 초등학교 시절 기억에 남는 순간 타이틀 제목 2줄까지 늘어나라 얍',
    description:
...
const ScrollingView = (): JSX.Element => {
  ...
  useEffect(() => {
    if(selectedCategory === 'all'){
      setData(storyViewData);
    }else{
      let filterData = storyViewData.filter(e => e.category === selectedCategory);
      setData(filterData);
    }
  }, [selectedCategory]);
  ...
  return (
    <SafeAreaView style={styles.scrollViewContainer}>
      <ScrollView
        ...
        {data.map((item:ItemData, index: number) => <StoryListItem data={item} key={index} />)}
      </ScrollView>
      {scrollPositionY >= 10 ? (
        <TouchableOpacity
